...
class Maze:
    ...
    def __init__(self) -> None:
        ...
        binFile:bool = bool(input("Binary file save: "))
        ...
        if qImage or sImage or qFrames or sFrames or qVideo or sVideo or objFile:
            ...
            border = list(map(int,input("Border color: ").split(" ")))
            bg = list(map(int,input("Background color: ").split(" ")))
            solBg = list(map(int,input("Solution color: ").split(" ")))
        if qVideo or sVideo:
        ...
        self.name:str = mazeName if mazeName else f'maze{self.seed}'
        ...
        if self.functionalities&self.sImageFx:
            self.saveImage(f'{self.dirname}',f'{self.name}SImage',self.sFrames[-1] if self.functionalities&self.sFramesFx else self.mazeCurrentImage(True))
